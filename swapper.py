...
def process(source_img: Union[Image.Image, List],
    ...
    # Detect faces to be replaced in the target image
    target_faces = get_many_faces(face_analyser, target_img)
    num_target_faces = len(target_faces) if target_faces else 0
    num_source_images = len(source_img) if isinstance(source_img, list) else 1

    if target_faces:
        temp_frame = copy.deepcopy(target_img)
        if isinstance(source_img, list):
            for i, single_source_img in enumerate(source_img):
                source_faces = get_many_faces(face_analyser, cv2.cvtColor(np.array(single_source_img), cv2.COLOR_RGB2BGR))
                num_source_faces = len(source_faces) if source_faces else 0

                if source_faces:
                    source_index = min(i, num_source_faces - 1)
                    target_index = min(i, num_target_faces - 1)

                    temp_frame = swap_face(face_swapper, source_faces, target_faces, source_index, target_index, temp_frame)
                else:
                    print(f"No faces found in source image {i}")
        else:
            # Single source image logic
            source_faces = get_many_faces(face_analyser, cv2.cvtColor(np.array(source_img), cv2.COLOR_RGB2BGR))
            num_source_faces = len(source_faces) if source_faces else 0

            if source_faces:
                for i in range(num_target_faces):
                    source_index = 0 if num_source_faces == 1 else min(i, num_source_faces - 1)
                    target_index = i
